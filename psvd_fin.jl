...
for (gesvd, elty, relty) in
  ...
  @eval begin
    ...
    function psvd_qr!(F::PSVD{$elty, $relty, M}, A::StridedMatrix{$elty}; full::Bool = false) where M
      jobuvt = full ? 'A' : 'S'
      m, n = size(A)
      m, n = size(A)
      minmn = min(m, n)
...
for (gesdd, elty, relty) in
  ...
  @eval begin
    ...
    # !!! this call destroys the contents of A
    function psvd_dd!(F::PSVD{$elty, $relty, M}, A::StridedMatrix{$elty}; full::Bool = false) where M
      job = full ? 'A' : 'S'
      m, n   = size(A)
      minmn  = min(m, n)
    #  @assert length(F.S) == minmn
