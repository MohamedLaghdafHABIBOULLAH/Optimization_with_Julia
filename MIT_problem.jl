...
Pkg.add(url = "https://github.com/MohamedLaghdafHABIBOULLAH/RegularizedProblems.jl", rev = "Matrix_completion")
...
# Definir l'image
I = ones(256,256); I[:,1:20] .= 0;I[1:126,40:60] .= 0; I[:,80:100] .= 0; I[1:40, 120 : 140] .= 0; I[80:256, 120 : 140] .= 0.5; I[1: 40, 160:256] .= 0; I[80:256, 160 : 180] .= 0;
...
X, B, Omega = Perturb(I, 2, 2);

# ╔═╡ c17b0eaf-fe93-4dac-b83a-d08a1ff3bfae
# Definir le modèle
function mat_rand_model2(m::Int, n::Int, B, Omega, xinit)
    res = zeros(m,n)

    function resid!(res, x)
        res .= zeros(m,n)
        res[Omega] .= B .- ShiftedProximalOperators.reshape_array(x, (m,n))[Omega]
        vec(res)
    end
    ...
    return FirstOrderModel(obj, grad!, xinit, name = "MIT") 
end
...
f = mat_rand_model2(256,256,B,Omega,xinit);
...
# Definir la solution
options = ROSolverOptions(ν = 1., β = 1e16, ϵ = 1e-6, verbose = 10, maxIter = 100);

# ╔═╡ 10359325-ffa2-45df-b0aa-75e3326bee5b
# Resoudre avec R2
sol = R2(f, hr, options, x0 = f.meta.x0)
